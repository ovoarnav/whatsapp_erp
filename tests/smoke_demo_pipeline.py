...
subprocess.run([sys.executable, "whatsapp_zip_to_jsonl.py", "--zip", "demo_data/demo_whatsapp_export.zip", "--out", "/tmp/smoke_msgs.jsonl", "--tz", "America/Toronto"], check=True)
subprocess.run([sys.executable, "extract_jobs_with_ai.py", "--in", "/tmp/smoke_msgs.jsonl", "--out", "/tmp/smoke_jobs.jsonl", "--csv", "/tmp/smoke_jobs.csv", "--model", "phi3.5"], check=True)
